...
class ModelData():
  def __init__(self, running = True, screen = pygame.display.set_mode((xScene, yScene)), edges = defaultdict(set)):
...
def main():
    ...
    dt = 0
    player_pos = pygame.Vector2(model.screen.get_width() / 2, model.screen.get_height() / 2)
    while running:
        # poll for events
...
def boxDrawer(model): #changes squareColor array into a grid with each color on that square
    for col in range(cols):
        for row in range(rows):
            pygame.draw.rect(model.screen, pygame.Color(0, 100, 0), pygame.Rect((boxW*col,boxH*row), (boxW,boxH)), width=borderW)#green borders, draw over with red if there is wall
        ...
        for row in range(rows):
            for i in range(4):
                if (col + dirs[i][0], row + dirs[i][1]) not in model.edges[(col, row)]:
                    ...
                    pygame.draw.line(model.screen, pygame.Color(100, 0, 0), topLeft, endPos, width=borderW)

def drawUI(col, row, model):
    topLeft = (0, int(4*model.screen.get_height()/5))
    theCell = pygame.Rect(topLeft, (model.screen.get_width(), int(model.screen.get_height()/5)))
    ...
    textPos = pygame.Rect((topLeft[0]+int(model.screen.get_width()/2-50), topLeft[1]+int(model.screen.get_height()/10)), (model.screen.get_width(), int(model.screen.get_height()/3)))
    model.screen.blit(text, textPos)
main()